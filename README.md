...
**It is read again every night on its own.** A scheduled workflow,
...
merges. A quiet night opens nothing. The cadence is the cron line at the top of
...
| `bin/diff-pool-info.mjs` | say what changed between two copies of `lib/poolinfo.js`, for the nightly crawl's PR |
| `.github/workflows/crawl-sundlaugar.yml` | read the whole directory every night and open a PR when it has changed |
