...
/* The closer look at the capital area: a centre and a width in kilometres rather
   ...
   It lives here rather than beside the drawing code because the coastline is
   cut for it at build time: bin/build-coastline.mjs clips and simplifies an
   outline specifically for this window, through frame() below, and the page
   draws that outline through the same frame(). Change the window and the
   generator has to run again — lib/poolmap.js refuses to draw a window it has
   no outline for rather than stretch the country's. */
