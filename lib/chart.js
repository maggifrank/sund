...
export function chartHTML(lang, trips, limit = CHART_MONTHS) {
  ...
  /* 320x124 and not the 320x168 this started at. The viewBox is scaled to the
     container by CSS, so the aspect ratio is the only thing deciding how tall
     the chart stands: at 1.9:1 a column of the desktop board came out 187px
     of bar, which made twelve monthly counts the heaviest thing on the page.
     2.6:1 is the band a dashboard wants, and the plot keeps 88 units of height
     for the tallest bar — enough that the axis ticks still separate. */
  const W = 320, H = 124;
  ...
  series.forEach((m, i) => {
    ...
    const y = yOf(m.count);
    /* Everything belonging to one month carries the same mark — the column, its
       number and its name on the axis — so hovering any of them lights the
       whole column and lets the other eleven recede. --i staggers the grow-in. */
    const mark = `data-mark="${i}" style="--i:${i}"`;
    if (m.count > 0) parts.push(`<path class="bar" ${mark} d="${barPath(cx - barW / 2, y, barW, h)}"/>`);
    // Label the peak only — a number on every column is noise.
    if (i === peak && m.count > 0) {
      parts.push(`<text class="bar-label" data-mark="${i}" x="${cx}" y="${y - 5}" text-anchor="middle">${m.count}</text>`);
    }
    if (i % labelEvery === 0 || i === series.length - 1) {
      parts.push(`<text class="axis-text" data-mark="${i}" x="${cx}" y="${H - 7}" text-anchor="middle">${svgEsc(formatDate(lang, m.date, 'short'))}</text>`);
    }
    ...
    parts.push(
      `<rect class="hit" data-mark="${i}" x="${PAD.left + band * i}" y="${PAD.top}" width="${band}" height="${plotH}" ` +
...
/* Hovering any mark lights everything keyed to it and lets the rest recede.

   One helper for both charts, because neither needs to know what the other is
   keyed on: the month chart marks a column, the pie a weekday, and the parts
   that belong together simply carry the same `data-mark`. Bound once to the
   container, which outlives the innerHTML the charts are rebuilt with.

   Pointer only, and deliberately: this is a second reading of what the legend
   and the tooltip already say outright, so nothing is lost on a device that
   cannot hover. */
export function bindChartHighlight(container) {
  let current = null;

  const apply = (mark) => {
    if (mark === current) return;          // pointermove fires far more than the state changes
    current = mark;
    for (const el of container.querySelectorAll('.is-hover')) el.classList.remove('is-hover');
    container.classList.toggle('is-hovering', mark !== null);
    if (mark === null) return;
    for (const el of container.querySelectorAll(`[data-mark="${CSS.escape(mark)}"]`)) {
      el.classList.add('is-hover');
    }
  };

  /* Reading the mark off whatever is under the pointer — rather than only off
     the marks themselves — is what clears the highlight when the pointer moves
     onto the heading or the gap between wedges, without waiting for it to leave
     the card altogether. */
  const onPoint = (e) => apply(e.target.closest('[data-mark]')?.dataset.mark ?? null);
  container.addEventListener('pointerover', onPoint);
  container.addEventListener('pointermove', onPoint);
  container.addEventListener('pointerleave', () => apply(null));
}
...
export function weekdayHTML(lang, trips) {
  ...
  for (const d of days) {
    ...
    /* The nudge each wedge takes when hovered: one step straight out along its
       own middle, handed to CSS so the geometry stays here and the motion stays
       there. A single wedge holding the whole circle has no outward to move
       along that reads as anything but a slipped drawing, so it gets none. */
    const [nx, ny] = d.share < 1 ? pointOn(0, 0, 4, (from + to) / 2) : [0, 0];
    wedges.push(
      `<path class="slice${d.top ? ' is-top' : ''}" data-mark="${d.day}" ` +
      `style="--nx:${nx}px;--ny:${ny}px;--i:${wedges.length}" d="${path}" role="img" ` +
    ...
    if (d.share >= 0.1) {
      labels.push(
        `<text class="slice-label${d.top ? ' is-top' : ''}" data-mark="${d.day}" ` +
        `style="--nx:${nx}px;--ny:${ny}px" x="${lx}" y="${ly}" ` +
  ...
  const legend = days.map((d) => (
    `<li class="weekday-row${d.top ? ' is-top' : ''}" data-mark="${d.day}">` +
