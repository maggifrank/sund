 ...
 * among them — are absent from the listing and were taken from there. The last
 * 24 are a section the pool directory does not reach at all: heitar_laugar, the
 ...
 *
 * Positions were then looked up in OpenStreetMap by name, which placed 81 of
 * the 131 and left fifty with nothing. Those fifty were not missing from OSM so
 * much as unnamed in it: the directory calls a pool after the town it stands
 * in, and OSM leaves the basins inside a sports complex untagged, so a name had
 * nothing to match.
 *
 ...
 * the water is — so 126 of the 131 can be detected by location. Four of those
 * OSM has no pool for at all, and stand at their address: Heydalur,
 * Íþróttamiðstöðin í Reykholti, Stapalaug and Reyðarfjörður. That pass is
 * bin/survey-pools.mjs: run it and diff it rather than take these on trust.
 *
 * The five still carrying a name and no position are the wild pools — Hveragil,
 * Kerið á Húsavíkurhöfða, Lægðin, Siká and Laugarnes – Birkimel — which are in
 * the natural-pools section, which lists no address, and which OSM does not
 * name. They cannot be auto-detected, but they appear in the picker, so a swim
 * can still be attributed to them from the History list.
 *
...
export const BUILT_IN = [
  ...
  { id: 'kerid-a-husavikurhofda', name: 'Kerið á Húsavíkurhöfða' },
  { id: 'kualaug', name: 'Kúalaug', lat: 64.3268, lon: -20.28195 },
  { id: 'laegdin', name: 'Lægðin' },
  ...
  { id: 'laugarnes-birkimel', name: 'Laugarnes – Birkimel' },
