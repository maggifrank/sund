...
/* How many faces show up for the party, and how far apart they land. One
   glyph in the corner of a small + button is easy to miss entirely; a little
   cluster in the middle of the screen is not. */
const EMOJI_BURST = 3;
const EMOJI_SPACING = 30;

/* One floating face, then it takes itself out of the document. Fixed rather
   than absolute: it lives for about a second, and anchoring it to the
   viewport means it does not have to care which panel it was launched from or
   how far that panel is scrolled. */
function spawnEmoji(emoji, offsetX, delay) {
  ...
  setTimeout(() => node.remove(), 2000 + delay);
}

/* A small burst from the middle of the screen rather than one face pinned to
   whatever was tapped: the tap already moved (the + button lives in a corner
   a thumb can reach, not where an eye is resting), so this is where a look
   lands next. Each face is its own pick() call, so the burst reads as a
   flourish of different expressions rather than the same one tripled. */
function popEmoji(kind) {
  for (let i = 0; i < EMOJI_BURST; i++) {
    const offsetX = (i - (EMOJI_BURST - 1) / 2) * EMOJI_SPACING;
    spawnEmoji(pick(kind), offsetX, i * 70);
  }
}
...
/* And one came out again. No confetti, no milestones — just a face that agrees
   with you about it. */
export function celebrateRemove({ counter } = {}) {
  haptic('remove');
  popEmoji('remove');
  bump(counter, 'down');
}
