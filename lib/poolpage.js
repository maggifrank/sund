...
import { guideFor } from './poolguide.js';
import { mapHTML, windowFor } from './poolmap.js';
import { poolHref } from './pools.js';
import { busyFor, busyChartHTML, busySummary, seasonOfMonth, BUSY_SOURCE } from './busychart.js';
import { bindChartTooltip } from './chart.js';
...
export function icelandToday(now = new Date()) {
  ...
  return { iso, md: iso.slice(5), weekday: now.getUTCDay(), hour: now.getUTCHours(), month: now.getUTCMonth() };
}
...
/* Which weekday each pool's busy-times chart was last turned to, so a redraw
   — a poll with a new swim in it, the hour ticking over — does not throw the
   reader back to today. */
const busyDay = new Map();

/* How busy the pool usually is through the day, where there are numbers to say
   so: Reykjavík's eight, from the city's own gate counts. The season is today's
   and the day starts as today, with the hour it is now picked out; the day
   buttons turn to any other. */
function busyCard(lang, id, today) {
  const busy = busyFor(id);
  if (!busy) return null;
  const season = seasonOfMonth(today.month);
  const order = [1, 2, 3, 4, 5, 6, 0];

  const chart = h('div', { class: 'chart busy-chart' });
  const nowLine = h('p', { class: 'busy-line busy-now' });
  const peakLine = h('p', { class: 'busy-line' });
  const days = h('div', { class: 'busy-days', role: 'group', 'aria-label': t(lang, 'busy.dayPick') },
    order.map((d) => h('button', { type: 'button', class: 'busy-day', 'data-day': String(d), text: weekdayName(lang, d, 'short') })));

  const draw = () => {
    const weekday = busyDay.get(id) ?? today.weekday;
    const hour = weekday === today.weekday ? today.hour : null;
    const args = { season, weekday, hour, dayName: weekdayName(lang, weekday, 'full') };
    chart.innerHTML = busyChartHTML(lang, busy, args);
    const said = busySummary(lang, busy, args);
    nowLine.textContent = said.now ?? '';
    nowLine.hidden = !said.now;
    peakLine.textContent = said.peak ?? '';
    peakLine.hidden = !said.peak;
    for (const button of days.children) {
      button.setAttribute('aria-pressed', String(Number(button.dataset.day) === weekday));
    }
  };
  days.addEventListener('click', (e) => {
    const button = e.target.closest('.busy-day');
    if (!button) return;
    busyDay.set(id, Number(button.dataset.day));
    draw();
  });
  bindChartTooltip(chart);
  draw();

  const span = `${formatDate(lang, `${BUSY_SOURCE.from}T12:00:00`, 'month')} – ${formatDate(lang, `${BUSY_SOURCE.to}T12:00:00`, 'month')}`;
  return card('pool-busy', t(lang, 'busy.label'),
    h('div', { class: 'sub', text: t(lang, `busy.season.${season}`) }),
    days, chart, nowLine, peakLine,
    h('p', { class: 'note' },
      `${t(lang, 'busy.note', { span })} `,
      h('a', { href: BUSY_SOURCE.url, target: '_blank', rel: 'noopener', text: BUSY_SOURCE.name })));
}
...
/* A natural pool's own card: the water, the way in and what to mind, from
   lib/poolguide.js. Most of these pools have no hours, prices or facilities
   on the directory, and without this their page was an address and a map. */
function guideCard(lang, guide) {
  if (!guide) return null;
  const num = (n) => String(n).replace('.', lang === 'en' ? '.' : ',');
  const facts = [
    guide.temp && ['guide.temp', guide.temp[0] === guide.temp[1]
      ? `${num(guide.temp[0])} °C`
      : `${num(guide.temp[0])}–${num(guide.temp[1])} °C`],
    guide.access && ['guide.access', t(lang, `guide.access.${guide.access}`)],
    guide.walk && (guide.walk.km || guide.walk.min) && ['guide.walk', [
      guide.walk.km ? `${num(guide.walk.km)} km` : null,
      guide.walk.min ? t(lang, 'guide.minutes', { min: guide.walk.min }) : null
    ].filter(Boolean).join(' · ')],
    guide.facilities && ['guide.facilities', t(lang, `guide.facilities.${guide.facilities}`)],
    guide.free !== null && guide.free !== undefined && ['guide.cost', t(lang, guide.free ? 'guide.free' : 'guide.paid')]
  ].filter(Boolean);
  const directions = guide.directions?.[lang] ?? guide.directions?.is ?? null;
  const cautions = guide.cautions ?? [];
  return card('pool-guide', t(lang, 'guide.label'),
    facts.length ? h('dl', { class: 'contact-list' }, facts.flatMap(([key, value]) => [
      h('dt', { text: t(lang, key) }),
      h('dd', { text: value })
    ])) : null,
    directions ? h('p', { class: 'pool-directions', text: directions }) : null,
    cautions.length ? h('ul', { class: 'pool-cautions' },
      cautions.map((c) => h('li', { text: t(lang, `guide.caution.${c}`) }))) : null);
}
...
function whereCard(lang, { info, pool, visits }) {
  ...
  /* A mark is drawn for the size it ends up on the screen, so the map has to be
     told how wide it will be. This one is a card wide on every screen, so it is
     drawn for a card in a phone column and measured once it is on the page. */
  const drawMap = (width) => {
  ...
  requestAnimationFrame(() => {
    const width = Math.round(map.getBoundingClientRect().width);
    if (width && Math.abs(width - 320) > 8) drawMap(width);
  });
  const gmaps = `https://www.google.com/maps/search/?api=1&query=${pool.lat},${pool.lon}`;
  return card('pool-where', t(lang, 'poolpage.where'),
    info?.address ? h('p', { class: 'pool-address', text: info.address }) : null,
    map,
    h('p', { class: 'pool-links' }, h('a', { href: gmaps, target: '_blank', rel: 'noopener', text: t(lang, 'poolpage.openMap') })),
...
/* A pool in a list: the map's own mark for it, its name, how often it has been
   swum in, and the chevron that says the row goes somewhere. Exported because
   every list of pools in the app is one of these — the region list here, and
   the map page's list of the ones still to go, which used to be the same row
   without the link. `tag` is the map page's "not on the map" pill. */
export function poolLinkRow(lang, { id, name, visits = 0, tag = null }) {
  return h('a', { class: 'pool-row pool-link', href: poolHref(id) },
    ...
    tag ? h('span', { class: 'pool-tag', text: tag }) : null,
    h('span', { class: 'pool-count', text: visits > 0 ? String(visits) : '' }),
    h('span', { class: 'chev', 'aria-hidden': 'true', text: '›' }));
}

const linkRow = (lang, id, name, visits) => poolLinkRow(lang, { id, name, visits });
...
export function renderPoolPage(root, lang, { id, pool, visits, rows, now = new Date() }) {
  ...
  const cards = [
    ...
    guideCard(lang, guideFor(id)),
    info && hoursCard(lang, info, today),
    busyCard(lang, id, today),
...
/* Callers skip the rebuild when this has not moved. The private page polls,
   ...
   and hour are in it because the page's idea of now — which row is highlighted,
   which closures still stand, which column of the busy-times chart is this hour
...
export function poolPageSignature(lang, { id, pool, visits, rows, now = new Date() }) {
  ...
  const near = region
    ...
    : [];
  return [lang, id, pool?.name ?? '', visits.count, visits.last ?? '', visits.card, icelandToday(now).iso, icelandToday(now).hour, ...near].join('|');
}
