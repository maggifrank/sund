...
function guideCard(lang, guide) {
  ...
  const facts = [
    ...
    guide.walk && (guide.walk.km || guide.walk.min) && ['guide.walk', [
      ...
      guide.walk.min ? t(lang, 'guide.minutes', { min: String(guide.walk.min) }) : null
    ].filter(Boolean).join(' · ')],
  ...
  const about = guide.about?.[lang] ?? guide.about?.is ?? null;
  ...
  return card('pool-guide', t(lang, 'guide.label'),
    ...
    about ? h('p', { class: 'pool-directions', text: about }) : null,
