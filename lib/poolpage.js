...
/* The blocks of hours that are in force today. A dated block is in force
   inside its dates. An undated one is in force only when nothing else of its
   kind on the page has dates — otherwise it is the other half of a year this
   cannot place, and marking it as today's would be a guess presented as a
   fact. */
function currentBlocks(hours, md) {
  return hours.filter((block) => {
    if (block.season) return inSeason(block.season, md) === true;
    return !hours.some((other) => other.kind === block.kind && other.season);
  });
}
...
function blockTitle(lang, block, hours) {
  ...
  if (block.kind === 'open' && block.season) {
    return block.season.to
      ? `${formatDayMonth(lang, block.season.from)} – ${formatDayMonth(lang, block.season.to)}`
...
/* What today's opening hours are, when that can be said: one block of opening
   ...
   closure without a start date might already have started, and "open today
   06:30–22:00" above a notice saying it is shut is worse than saying nothing. */
function todayLine(lang, info, today) {
  const open = currentBlocks(info.hours.filter((b) => b.kind === 'open'), today.md);
  if (open.length !== 1) return null;
  if (info.hours.some((b) => activeNotes(b.notes, today.iso).some((n) => n.until))) return null;
  const rows = open[0].rows.filter((r) => r.days?.includes(today.weekday));
...
function hoursCard(lang, info, today) {
  ...
  const blocks = info.hours.map((block) => {
    ...
    const rows = block.rows.map((row) =>
      h('tr', { class: now && row.days?.includes(today.weekday) ? 'is-today' : null },
        h('th', { scope: 'row', text: row.days ? daysLabel(lang, row.days) : (row.label ?? '') }),
    ...
    return h('div', { class: `hours-block${now && seasonal && block.season ? ' is-current' : ''}` },
      ...
      notes.map((n) => h('p', { class: n.until ? 'pool-notice' : 'hours-note', text: n.text })));
  });
...
function whereCard(lang, { info, pool, visits }) {
  ...
  /* The capital area at a little over half its width, centred on the pool: the
     neighbourhood rather than the whole bay. Anywhere else is the country map
     as it is, whose outline is too coarse to be looked at any closer. */
...
function contactCard(lang, info) {
  ...
  const entries = [
    info.phone && ['poolpage.phone', `tel:${info.phone.replace(/\s/g, '')}`, info.phone],
